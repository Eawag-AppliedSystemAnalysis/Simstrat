...
!######################################################################
      ...
      if (NBC==1) then
          ...
          if (NBC/=4) then ! in the water column
              ...
              fu = sqrt((2.7*max(0.0,(T(xl)-T_atm)/(1-0.378*Vap_atm/p_air))**0.333)**2 + (0.6072*3.1*wind)**2)
              ...
              H_A = (1-r_a) * ((1-0.84*Cloud) * (59.38 + 113.7*((T_atm+273.15)/273.16)**6&
              &   + 96.96*sqrt(465*Vap_atm/(T_atm+273.15)/25)) / 5.67e-8 / &
              &   (T_atm+273.15)**4 + 0.84 * Cloud) *5.67e-8*(T_atm+273.15)**4
      ...
      if (ModC10==1) then !constant wind drag coefficient
          C10 = C10_const
          ...
          if (wind<=0.1) then !MS 2016 added C10_const as calibration parameter
              ...
              C10 = C10_const*0.0044*wind**(-1.15)
          else !Polynomial approximation of Charnock's law
              C10 = C10_const*(-0.000000712*wind**2+0.00007387*wind+0.0006605)
