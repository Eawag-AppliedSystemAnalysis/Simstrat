      ...
      associate (datum=>state%datum, &
        ...
        do i=1,4
          if (idx) then
            ...
            ! Check whether there are any surface inputs, otherwise assume that all values are for deep inputs
            ...
            if (i==3 .or. i==4) then
               if (any(self%z_Inp(i,1:self%nval_deep(i))/=self%z_Inp(1,1:self%nval_deep(i)))) then
