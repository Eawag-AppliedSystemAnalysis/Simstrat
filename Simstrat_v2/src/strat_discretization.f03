module strat_discretization
  ...
  contains
    ...
    subroutine generic_create_LES(self, var,nu,  sources,  lower_diag,main_diag, upper_diag,  rhs, dt)
      class(Discretization), intent(inout) :: self
      real(RK), dimension(:), intent(inout) :: var,  sources,  lower_diag, upper_diag, main_diag, rhs,nu
      real(RK), intent(inout) :: dt

    end subroutine


    subroutine euleri_create_LES(self, var, nu, sources,  lower_diag, main_diag, upper_diag,  rhs, dt)
      ...
      lower_diag(n) = 0.0_RK
      main_diag(1:n) = 1.0_RK - upper_diag(1:n) - lower_diag(1:n)
