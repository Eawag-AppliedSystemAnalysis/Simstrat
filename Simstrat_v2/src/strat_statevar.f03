module strat_statevar
  ...
  contains
    subroutine generic_var_init(self, grid, solver, disc, nu, var)
    ...
    subroutine generic_var_calc_terms(self, state, param, sources)
      class(ModelVariable), intent(inout) :: self
    ...
    subroutine generic_var_update(self, state, param)
      ...
      class(ModelState), intent(inout) :: state
      class(ModelParam), intent(inout) :: param
      real(RK), dimension(size(self%var)) :: main_diag, rhs, sources, lower_diag, upper_diag

      call self%calc_terms(state, param, sources)
      call self%disc%create_LES(self%var, self%nu, sources,  lower_diag, main_diag, upper_diag , rhs, state%dt)
