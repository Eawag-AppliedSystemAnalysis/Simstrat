...
contains
  ...
  subroutine eps_var_calc_terms(self, state, param, sources, boundaries)
    ...
    state%avh(1)  = 0.5_RK/sig_k*(0+state%num(1))

    if (self%cfg%flux_condition==1 .and. self%cfg%turbulence_model == 1) then
        flux(1) = state%avh(1)*(state%cde*((state%ko(1))**1.5_RK))/(kappa*(K_s+0.5_RK*grid%h(1)))**2
        flux(ubnd_fce) = state%avh(ubnd_fce)*(state%cde*((state%ko(ubnd_fce))**1.5_RK))/(z0*(K_s+0.5_RK*grid%h(ubnd_vol)))**2
        do i=2,ubnd_fce -1
            flux(i) = state%num(i)/sig_e*(state%cde*((state%ko(i))**1.5_RK)/(kappa*(z0+0.25_RK*(grid%h(i)+grid%h(i+1))))**2)
        end do
        state%avh(1) = 0.0_RK
        state%avh(ubnd_fce) = 0.0_RK
    else
        state%avh(1)=2*state%u_taub**4/sig_e/(state%eps(1)+state%eps(2))        ! = 0 for no shear stress
        state%avh(ubnd_fce)=2*state%u_taus**4/sig_e/(state%eps(ubnd_fce)+state%eps(ubnd_fce-1))   ! = 0 for no shear stress
    end if
    ...
    if(self%cfg%flux_condition==1 .and. self%cfg%turbulence_model == 1) then
      sources(2:ubnd_fce-1) = flux(2:ubnd_fce-1)*grid%AreaFactor_eps(2:ubnd_fce-1) ! AreaFactor_eps = 1/A * dA/dz (at epsilon posions)
      if (grid%Az(1)/=0) then                          ! Flux from bottom only!
          sources(2)= flux(1)*(grid%Az(1)+grid%Az(2))/(grid%Az(2)*(grid%h(1)+grid%h(2)))
      end if
      sources(ubnd_fce-1)= flux(ubnd_fce-1)*(grid%Az(ubnd_fce)+grid%Az(ubnd_fce-1))/(grid%Az(ubnd_fce-1)*(grid%h(ubnd_fce-1)+grid%h(ubnd_fce-2)))
  ...
  subroutine eps_var_post_solve(self, state, param)
    ...
    ! Define eps at boundary (no flux)
      ...
      self%var(ubnd_fce)=   self%var(ubnd_fce-1)+(state%cde*((state%ko(ubnd_fce-1))**1.5_RK)/(kappa*(z0 +grid%h(ubnd_fce-1)))**2)*grid%h(ubnd_fce-1)
    end if
