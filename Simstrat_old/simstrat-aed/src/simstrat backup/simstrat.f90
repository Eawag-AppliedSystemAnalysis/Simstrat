...
contains
    ...
    subroutine simstrat_simulation()
        ...
        do while (datum<t_end)
            ...
            if (igoal<0) then
                i_av=i_av+1
                call avstate(i_av,U,V,T,S,k,eps,num,nuh,B,P,NN,P_Seiche,E_Seiche,U_av,V_av,&
            ...
            if ((itera==write_tout).or.(datum>=t_end)) then
                itera=0
                if (igoal<0) then
                    call prep_outav(i_av,U_av,V_av,T_av,S_av,k_av,eps_av,num_av,nuh_av,B_av,P_av,NN_av,P_Seiche_av,E_Seiche_av)
                    call write_out_new(datum,U_av,V_av,T_av,S_av,k_av,eps_av,nuh_av,B_av,P_av,NN_av,P_Seiche_av,E_Seiche_av,Q_vert)
                    i_av=0
                else
                    if (OutBin) then
                        call write_out_new(datum,U,V,T,S,k,eps,nuh,B,P,NN,P_Seiche,E_Seiche,Q_vert)
                    else
                        call write_text(datum,U,V,T,S,k,eps,nuh,B,P,NN,P_Seiche,E_Seiche,Q_vert)
                    end if
                end if
                if(disp_sim==1) write(6,991) datum,z_upp(nz),T(nz),T(1)
            end if
