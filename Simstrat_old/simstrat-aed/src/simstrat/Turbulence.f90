...
contains
    ...
    !####################################################################
        ...
        write(*,*) "SUMP=",sum(P(1:nz-1))
        P(1:nz-1) = P(1:nz-1)*num(1:nz-1)*meanint(1:nz-1)**2
        write(*,*) "SUMP2=",sum(P(1:nz-1))
    ...
    !####################################################################
        ...
        if (a_seiche/=0.) then      ! a_seiche is defined in the par-file
            do i=1,nz-1
                ...
                write(*,*) "NN(",i,"):",NN(i),",",q_NN,",",NN(i)**q_NN
...
write(*,*) "SizeAVH=",size( avh(1:nz))
        ...
        write(*,*) "FLUX=",flux(0:nz)
        write(*,*) "SIZEFLUX=",size(flux)
        ...
        do i=1,nz-1
            ...
            Prod=ce1*eps(i)/ko(i)*(P(i)+P_Seiche(i))        ! New code plus seiche
              ...
              !!DEBUG!!
              Prod=ce1*eps(i)/ko(i)*(P(i))        ! New code plus seiche
              !! END DEBUG!!
            ...
            if (Prod+Buoy>0) then
                ...
                pminus(i)=Diss-Buoy
                    ...
                    write(*,*) "BPD(",i,")", Buoy, ",",Prod, ",",Diss,",",eps(i), ",",ko(i),",",P(i)
        end do
        ...
        write(*,*) "pplus=[",pplus(1:nz-1),"]"
        write(*,*) "pminus=[",pminus(1:nz-1) ,"]"
        ...
        if ((ModFlux==1).and.(Mod==1)) then
          ...
          !  du(nz-1)= du(nz-1)+flux(nz)*dt*(Az(nz)+Az(nz-1))/(Az(nz-1)*(h(nz)+h(nz-1)))
            write(*,*) "FF:",flux(nz)," ",(Az(nz)+Az(nz-1))/(Az(nz-1)*(h(nz)+h(nz-1)))
        ...
        do i=0,nz
            if (NN(i)>0) then
              ...
              !  epslim = eps_min
            !    write(*,*) " i=",i," k=",k(i)," NN=",NN(i)
            else
                epslim= eps_min
            !    write(*,*) " i=",i," eps_min=",eps_min
            end if
            if(eps(i)<epslim) then
              eps(i)=epslim
            !  write(*,*) "i=",i," & epslim=", epslim
            end if
            if (eps(i)<0) then
