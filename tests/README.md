...
The FABM configuration file is called by `simstrat_fabm.f90` in create_model() and the output of the simulation is stored in `TestCases_Results`. The testcase uses the `fabm-gotm-npzd.yaml` configuration from the default FABM testcases under `../lib/fabm/testcases`. For details and further FABM configurations, consult `README_FABM.md`.
